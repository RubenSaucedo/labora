...
if (!runDir || !prefsPath) {
  process.stderr.write(
    "Usage: labora search merge <run-dir> --prefs <search-preferences.json> [--persona name] [--min-agreement 2] [--threshold 70] [--fit-floor 60] [--seen <seen.json>] [--suppress-seen]\n"
  );
  process.exit(1);
}
...
try {
  ...
  // Fit reports used to be checked against a compiled claim ledger. There is no
  // ledger any more, and a scout's fit judgment was never a deterministic fact
  // in the first place -- it is one reader's opinion about a posting, which the
  // operator reads and weighs. What survives here is the part that genuinely
  // needs code: canonical IDs, deduplication, consensus and ranking.
